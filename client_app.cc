...
ClientApp::ClientApp () : mouse_ (window_.mouse_), keyboard_ (window_.keyboard_), tickrate_ (1.0 / 60.0), input_bits_ (0), globalTick (0) {
...
bool ClientApp::on_tick (const Time& dt) {
    ...
    while (accumulator_ >= tickrate_) {
        ...
        if (positionHistory.size () > 0) {
            if (positionHistory[0].tick - tickrate_.as_ticks () > 200) {
...
void ClientApp::ServerDiscovery () {
    charlie::network::ServerDiscovery ();
}
